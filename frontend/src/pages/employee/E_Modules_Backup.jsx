    ...
    const fetchWorkstreams = useCallback(async () => {
        ...
        try {
            ...
            if (Array.isArray(response.data) && response.data.length > 0) {
                // Show all workstreams regardless of chapter count or published status
                // But we'll mark unpublished ones for display purposes
                const allWorkstreams = response.data.map(ws => ({
                    ...ws,
                    is_available: ws.is_published && ws.chapters_count > 0
                }));
                
                setWorkstreams(allWorkstreams);
                
                // Only show error if there are absolutely no workstreams
                if (allWorkstreams.length === 0) {
                    setError('No workstreams available. Please contact your administrator.');
                } else {
                    setError('');
                }
            } else {
        ...
        <div className="page-container">
            <div className="grid-container-ws">
                {currentWorkstreams.map((ws) => {
                    ...
                    const isAvailable = ws.is_available !== false; // Default to true if not specified

                    // Determine card status and action button
                    let statusBadge = null;
                    let actionButton;
                    
                    if (!isAvailable) {
                        statusBadge = <span className="status-badge unpublished">Unpublished</span>;
                        actionButton = <button className="action-btn no-content" disabled>Not Available</button>;
                    } else if (!hasContent) {
                        statusBadge = <span className="status-badge coming-soon">Coming Soon</span>;
                        actionButton = <button className="action-btn no-content" disabled>Check Back Later</button>;
                    } else if (isCompleted) {
                    ...
                    return (
                        <div 
                            ...
                            className={`card-ws ${!isAvailable || !hasContent ? 'inactive' : ''}`}
                            onClick={() => (isAvailable && hasContent) ? handleSelectWorkstream(ws) : null}
                        >
                            <div className="card-ws-image-container">
                                {ws.image_type ? 
                                    <img 
                                        src={`${API_URL}/workstreams/${ws.workstream_id}/image`} 
                                        alt={ws.title} 
                                        className="card-ws-image"
                                    /> :
                            ...
                            <div className="card-ws-content">
                                ...
                                {hasContent && isAvailable ? (
                                    ...
                                    <div className="no-content-container">
                                        <p>{!isAvailable ? 'This workstream is not yet published' : 'Content is being prepared'}</p>
                                    </div>
                                )}
